...
export class BibleUp {
	...
	/**
	 * {desc} get json data through XMLHttpRequest
	 * api endpoint - script/getscripture.php
	 */
	async fetchData(bibleRef, version) {
		let book = bibleRef['book'];
		let chapter = this.trimAll(bibleRef['chapter']);
		let verse = this.trimAll(bibleRef['verse']);
	...
	*/
	...
	setStage($element, $options) {
		...
		e.forEach(function(e) {
			...
			e.addEventListener('mouseleave', this.closePopup.bind(this));
		}, this);
	}
...
async clickb(e) {
	...
	let res = await this.fetchData(bibleRef, this.options.version); //res is fetch response result which is an object
...
closePopup(e) {
	...
	let t = e.target.id;
	//alert(mouseFrom.getAttribute('class'));
