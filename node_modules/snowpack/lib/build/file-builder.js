...
class FileBuilder {
    ...
    verifyRequestFromBuild(type) {
        ...
        if (!this.resolvedOutput[type] || !Object.keys(this.resolvedOutput).length) {
            throw new Error(`${this.loc} - Requested content "${type}" but built ${Object.keys(this.resolvedOutput).toString() || 'none'}.`);
        }
    ...
    async resolveImports(isResolve, hmrParam, importMap) {
        ...
        const pkgSource = util_1.getPackageSource(this.config.packageOptions.source);
        ...
        for (const [type, outputResult] of Object.entries(this.buildOutput)) {
            ...
            const resolveImport = async (spec) => {
                ...
                if (!resolvedImportUrl) {
                    try {
                        return await pkgSource.resolvePackageImport(this.loc, spec, this.config, importMap || (isResolve ? undefined : { imports: {} }));
                    }
                    catch (err) {
