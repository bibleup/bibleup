...
exports.command = exports.startServer = exports.OneToManyMap = void 0;
...
async function startServer(commandOptions, { isDev: _isDev, isWatch: _isWatch, preparePackages: _preparePackages, } = {}) {
    const isDev = _isDev !== null && _isDev !== void 0 ? _isDev : true;
  ...
  - ${colors.cyan('mkcert')}: ${colors.yellow('mkcert -install && mkcert -key-file snowpack.key -cert-file snowpack.crt localhost')}
    ...
    async function loadUrl(reqUrl, { isSSR: _isSSR, isHMR: _isHMR, isResolve: _isResolve, encoding: _encoding, importMap, } = {}) {
        ...
        if (reqPath === build_import_proxy_1.getMetaUrlPath('/env.js', config)) {
            return {
                contents: encodeResponse(build_import_proxy_1.generateEnvModule({ mode: 'development', isSSR, configEnv: config.env }), encoding),
                imports: [],
                originalFileLoc: null,
                contentType: 'application/javascript',
            };
        ...
        if (reqPath.startsWith(PACKAGE_PATH_PREFIX)) {
            ...
            if (reqUrl.split('.').length <= 2 && config.packageOptions.source !== 'remote') {
                ...
                const redirectUrl = await pkgSource.resolvePackageImport(path_1.default.join(config.root, 'package.json'), reqUrl.replace(PACKAGE_PATH_PREFIX, '').replace(/\.js/, ''), config);
            ...
            if (!symlinkDirectories.has(symlinkResourceDirectory)) {
                ...
                for (const f of shallowFiles) {
                    if (fileToUrlMapping.value(f)) {
                        ...
                        fileToUrlMapping.add(f, file_urls_1.getBuiltFileUrls(f, config).map((u) => path_1.default.posix.join(config.buildOptions.metaUrlPath, 'link', slash_1.default(path_1.default.relative(config.workspaceRoot, u)))));
                    }
                }
            }
        ...
        const cacheKey = util_2.getCacheKey(fileLoc, { isSSR, env: process.env.NODE_ENV });
    ...
    async function onWatchEvent(fileLoc) {
        ...
        inMemoryBuildCache.delete(util_2.getCacheKey(fileLoc, { isSSR: true, env: process.env.NODE_ENV }));
        inMemoryBuildCache.delete(util_2.getCacheKey(fileLoc, { isSSR: false, env: process.env.NODE_ENV }));
    ...
    const sp = {
        ...
        getUrlForPackage: (pkgSpec) => {
            return pkgSource.resolvePackageImport(path_1.default.join(config.root, 'package.json'), pkgSpec, config);
        },
