...
async function scanImports(includeTests, config) {
    ...
    const includeFileSets = await Promise.all(Object.keys(config.mount).map(async (fromDisk) => {
        return (await new fdir_1.fdir().withFullPaths().crawl(fromDisk).withPromise());
    }));
