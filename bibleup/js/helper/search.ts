...
const getText = async (ref: string, versionId: string): Promise<string[] | null> => {
 ...
 **/
const getPassage = async (ref: string, versionId: string): Promise<string[] | null> => {
 ...
 */
const processBibleText = (res: ApiResponse, type: 'text' | 'passage'): string[] => {
  const result: string[] = [];
  ...
  if (type === 'text') {
    ...
    result.push(p[0].textContent || p[1].textContent || 'Unable to process Bible text');
  }
    ...
    for (const verse of span) {
      ...
      if (currVerse !== lastVerse) {
        ...
        if (verse.textContent) {
          result[result.length - 1] = `${result[result.length - 1]} ${verse.textContent}`;
        }
      }
    }
  }
